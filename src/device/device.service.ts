...
export class DeviceService {
  ...
  async findByCertSerial(serial: string) {
    return this.certRepository.findOne({ where: { serialNumber: serial }, relations: ['device'] });
  }

  async isCertRegisteredForDevice(deviceId: string, serial: string) {
    const c = await this.certRepository.findOne({ where: { serialNumber: serial, device: { id: deviceId } } });
    return !!c;
  }
  ...
  async registerCert(deviceId: string, serial: string, subject: string, validFrom: Date, validTo: Date) {
    const device = await this.deviceRepository.findOne({ where: { id: deviceId } });
    if (!device) throw new Error('Device not found');
    const cert = this.certRepository.create({ serialNumber: serial, subject, validFrom, validTo, device });
  ...
  async createDevice(ownerId: string, dto: CreateDeviceDto): Promise<DeviceEntity> {
  ...
  async verifyAndIssue(deviceId: string, csrPem: string, nonce: string, signatureB64: string) {
    ...
    await this.ds.query(
      ...
      throw new InternalServerErrorException('failed to store cert metadata: ' + (err.message || err));
    });
  ...
  private async signCsrWithCa(csrPem: string) {
    ...
    try {
      ...
      const args = ['x509', '-req', '-in', csrPath, '-CA', caCertPath, '-CAkey', caKeyPath, '-CAcreateserial', '-out', outCert, '-days', '365', '-sha256', '-extfile', extFile];
