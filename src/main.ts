...
async function bootstrap() {
  ...
  const rawOrigins = configService.get<string>('FRONTEND_ORIGINS') || process.env.FRONTEND_ORIGINS || 'http://localhost:3000';
  ...
  const swaggerEnabled = (configService.get<string>('SWAGGER_ENABLE') || process.env.SWAGGER_ENABLE || 'false').toLowerCase() === 'true';
  if (swaggerEnabled) {
  ...
  const port = Number(configService.get<number>('APP_PORT')) || Number(process.env.APP_PORT) || 2008;
  ...
  try {
    ...
    console.log('could not read server.address():', (err as any)?.message || err);
  }
}
